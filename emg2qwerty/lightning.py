...
class LSTMAttentionCTCModule(CTCModule):
    """BiLSTM encoder followed by temporal self-attention over T.

    Architecture:
        SpectrogramNorm
        → MultiBandRotationInvariantMLP
        → Flatten
        → LSTMEncoder          (T, N, 2*hidden_size)
        → TemporalAttention    (T, N, 2*hidden_size)  ← multi-head self-attn over T
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            # Temporal self-attention: every timestep attends to all others
            TemporalAttention(
